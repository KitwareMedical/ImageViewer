...
class ITK_EXPORT FileIOMetaImageMaker : public FileIOMaker
{
public:
  FileIOMetaImageMaker() : FileIOMaker(m_MyObject->GetSupportedFileExtensions()) {}

	FileIO* MakeObject(const std::string& parameters) const
	{
    return FileIOMetaImage::New();
	}

protected:
	typedef FileIOMetaImageMaker myMakerType;
	typedef FileIOMetaImage myProductType;

  static const myMakerType* m_RegisterThis;
	static const myProductType* m_MyObject;
