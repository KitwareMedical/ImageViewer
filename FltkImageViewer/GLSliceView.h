...
GLSliceView<ImagePixelType, OverlayPixelType>::update()
{
  ...
  int winWidth = (int)( cDimSize[ cWinOrder[0] ] / (2 * cWinZoom) );
  cWinSizeX = ( (int) winWidth ) * 2;
  int ti = (int)( (int)cWinCenter[ cWinOrder[0] ] - winWidth);
  if( ti < -winWidth ) 
  {
    ti = -winWidth;
  }
  ...
  winWidth = (int)( cDimSize[ cWinOrder[1] ] / (2 * cWinZoom) );
  cWinSizeY = ( (int) winWidth ) * 2;
  ti = (int)( (int)cWinCenter[ cWinOrder[1] ] - winWidth);
  if( ti < -winWidth ) 
  {
    ti = -winWidth;
  }
...
void GLSliceView<ImagePixelType, OverlayPixelType>::draw(void)
{
  ...
  else
  {
    ...
    if( cViewValue ) 
    {
      glEnable(GL_BLEND);
      glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
      glColor4f(0.1, 0.8, 0.1, (float)0.9);
      gl_font(FL_TIMES_BOLD, 12);
      char s[80];
    ...
    if( cViewDetails )
    {
      glEnable(GL_BLEND);
      glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
      glColor4f(0.8, 0.1, 0.1, (float)0.9);
      gl_font(FL_TIMES_BOLD, 12);
      char s[80];
      ...
      sprintf(s, "Click Mode: %s", ClickModeTypeName[cClickMode]);
      gl_draw( s, 2, 2+0*(gl_height()+2) );
      glDisable(GL_BLEND);
    }
    if( cViewCrosshairs 
       && cClickSelect[cWinOrder[2]] == sliceNum())
    {
      glEnable(GL_BLEND);
      glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
      glColor4f(0, 1, 0, (float)0.5);
      int x = (int)(((cFlipX[cWinOrientation]
                      *(cW-(float)cClickSelect[cWinOrder[0]]) 
                     + (1-cFlipX[cWinOrientation])
                      *(float)cClickSelect[cWinOrder[0]]) 
                    - cWinMinX) * scale0);
      int y = (int)(((cFlipY[cWinOrientation]
                      *(cH-(float)cClickSelect[cWinOrder[1]])
                     + (1-cFlipY[cWinOrientation])
                      *(float)cClickSelect[cWinOrder[1]]) 
                    - cWinMinY) * scale1);
      glBegin(GL_LINES);
      glVertex2d(0, y);
      glVertex2d(x-2, y);
