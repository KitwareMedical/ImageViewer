...
namespace li 
{
/** This class emulates the imaging system of the fluoroscopy unit */
...
public:
  ...
  typedef enum {  TextureNotLoaded,
                  TextureLoaded,
                  TextureBinded } TextureState;
...
private:
  /**  Position of the source of X-rays. it will be used as the eye 
   *   position for OpenGL drawing. */
  PointType m_SourcePosition;

  /** Position of the Image Detector (or Film). It will be used as the eye
   *  image position for OpenGL drawing */
  PointType     m_DetectorPosition;

  /** Radius of the Image Detector (or Film). It will be used to compute the
   *  fieldOfView. */
  double        m_DetectorRadius;

  /** Direction associated with the vertical from the point of view of the
   *  detector. It will be used as the up image position for OpenGL drawing */
  VectorType    m_DetectorVerticalDirection;

  /**  Digital Zoom of the image Detector. */
  double        m_Zoom;

  /** This light object hols all the observers that correspond to commands
   *  that will redraw objects in OpenGL.  */
  itk::Object::Pointer  m_Notifier;

  /**  Real fluorosocpy Image  */
  ImageType::ConstPointer        m_Image;

  /**  Temporary image for fast OpenGL drawing and up side down correction */
